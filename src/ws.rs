...
use actix_web_actors::ws::{
    CloseCode, CloseReason, Message, ProtocolError, WebsocketContext, WsResponseBuilder,
};
...
use crate::{close_reason::CloseReason as ServerCloseReason, server::IncomingMessage};
use log::{error, warn};
// use semver::Version;
use std::time::{Duration, Instant};
use uuid::Uuid;

use crate::server::{OutgoingMessage, Server};
...
pub const SUBPROTOCOL: &str = "thirteen-game";
...
impl Actor for WsConnection {
    ...
    fn started(&mut self, ctx: &mut Self::Context) {
        ctx.run_later(HEARTBEAT_INTERVAL, |ws, ws_ctx| {
            ...
            if ws.uuid.is_nil() {
                ws_ctx.close(convert(ServerCloseReason::AUTH_FAILED));
                ws_ctx.stop();
                return;
            }
        });
        ctx.run_interval(HEARTBEAT_INTERVAL, |ws, ws_ctx| {
            ...
            if Instant::now().duration_since(ws.last_hb) > CLIENT_TIMEOUT {
                ...
                // // Close the actor.
                ws_ctx.close(convert(ServerCloseReason::HB_FAILED));
                ws_ctx.stop();
            } else {
                // Send a ping.
                ws_ctx.ping(&[]);
...
impl StreamHandler<OutgoingMessage> for WsConnection {
...
fn convert(close_reason: ServerCloseReason) -> Option<CloseReason> {
...
impl StreamHandler<Result<Message, ProtocolError>> for WsConnection {
    fn handle(&mut self, msg: Result<Message, ProtocolError>, ctx: &mut Self::Context) {
        ...
        //         }
                ...
                ctx.close(convert(ServerCloseReason::NO_AUTH));
                ctx.stop();
            }
        } else {
            match msg {
